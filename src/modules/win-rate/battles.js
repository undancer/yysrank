...
export const beautifyOutput = (list, input) =>
  _.sortBy(list, index =>
    input.indexOf(Number.parseInt(index)) >= 0
      ? input.indexOf(Number.parseInt(index))
      : Number.parseInt(index)
  );

const reports = (banList, thisTeamList, thatTeamList) => {
  let results = HeroBattleTable()
    .data.filter(
      battle =>
        _.isEmpty(banList) ||
        !(isContained(banList, battle.w) || isContained(banList, battle.l))
    ) // 首先排除ban选的式神
    .filter(battle => !_.isEqual(battle.w, battle.l)) // 排除胜负双方阵容一致的
    .filter(
      battle =>
        _.isEmpty(thisTeamList) ||
        isContained(thisTeamList, battle.w) ||
        isContained(thisTeamList, battle.l)
    ) // 只看自己场的式神
    .filter(
      battle =>
        _.isEmpty(thatTeamList) ||
