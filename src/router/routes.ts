...
const Rank = () => import("@/modules/rank/components/Rank");
const Recommend = () => import("@/modules/recommend/components/Recommend.vue");
const MoreTools = () =>
  import("@/modules/page/components").then(page => page.MoreTools);
const Tutorial = () =>
  import("@/modules/page/components").then(page => page.Tutorial);
...
const routes = [
  {
    ...
    meta: {
      title: "",
      subTitle:
        "数据来源于各服前百名斗技记录，每周一 12:00 ~ 14:00 更新，上次更新：" +
        config.update
    }
  },
  {
    ...
    meta: {
      title: "式神外战每周胜率榜",
      subTitle:
        "数据来源于各服前百名斗技记录，仅列出场次大于100的式神, 上次更新：" +
        config.update
    }
  },
  {
